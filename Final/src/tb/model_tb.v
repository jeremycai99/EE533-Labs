/* file: model_tb.v
 * ANN-only testbench — drives ONLY the NetFPGA RX/TX interface.
 * All computation runs inside soc (real CPU, CP10, DMA, GPU).
 * No backdoor register/memory pokes into u_soc.*.
 *
 * 5 tests, all exercise the 40→96→42→4 ANN or its sub-paths:
 *   T1  ANN 2-layer WMMA inference sanity   (hardcoded 4-vector, Y=16.0)
 *   T2  ANN full-model staging              (thread-1 weight slice, single-bank load path)
 *   T3  ANN full-model GPU load             (bank-striped, burst_all load path)
 *   T4  ANN full-model tiled inference      (515 host-driven 4x4 WMMA tiles)
 *   T5  HW scheduler (PRIMARY TEST)         — full-model hardware-owned flow:
 *         CPU-side scheduler runs the 36-tile outer loop and drives DMA + GPU
 *         entirely on the SoC, host only injects three packets and checks
 *         final logits.  No back-door pokes anywhere.
 *
 * Non-ANN tests (CPU ISA, GPU ISA, network passthrough, DMA unit tests)
 * live in soc_tb.v. This file is reserved for ANN model verification only.
 *
 ...
 * Author: Jeremy Cai
 * Date: Apr. 21, 2026
 * Version: 2.0 — stripped to ANN only; removed SOC_TB_MODEL_ONLY guards
 *                 and backdoor regfile pokes in Test 26.
 */
...
begin
    ...
    // v2.0: first two instructions read the CPU thread ID from CR10 into R11,
    // so the CMP-R11-#0 + B.NE-halt gate runs thread 0 only.  No backdoor.
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mrc(4'd11, 4'd10);                ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 0
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_cmp_imm(4'd11, 8'd0);              ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 1
    ann_hw_copy_prog[ann_hw_copy_prog_len] = 32'd0;                                  ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 2 — patched below
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mov_imm(4'd1, 8'd16);               ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 3
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mcr(4'd1, 4'd0);                    ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 4
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mov_imm(4'd0, 8'd0);                ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 5
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_ldr_imm(4'd2, 4'd0, 12'd0);         ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 6
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_ldr_imm(4'd3, 4'd0, 12'd4);         ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 7
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mcr(4'd2, 4'd1);                    ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 8
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mcr(4'd3, 4'd2);                    ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 9
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mov_imm(4'd4, 8'd65);               ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 10
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mcr(4'd4, 4'd3);                    ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 11
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mov_shift(4'd6, 4'd3, `SHIFT_LSL, 5'd2); ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 12
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_add_imm(4'd6, 4'd6, 8'd16);         ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 13
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_mrc(4'd5, 4'd9);                    ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 14
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_cmp_reg(4'd5, 4'd6);                ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 15
    ann_hw_copy_prog[ann_hw_copy_prog_len] = arm_b_idx(4'h1, 16, 14);                ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 16
    ann_hw_copy_prog[ann_hw_copy_prog_len] = ARM_HALT;                                ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 17
    ann_hw_copy_prog[ann_hw_copy_prog_len] = ARM_HALT;                                ann_hw_copy_prog_len = ann_hw_copy_prog_len + 1; // 18
    ann_hw_copy_prog[2] = arm_b_idx(4'h1, 2, ann_hw_copy_prog_len - 2);
...
begin
    ...
    // v2.0: first two instructions read CR10 (per-thread ID) into R11, so
    // the CMP-R11-#0 + B.NE-halt gate runs thread 0 alone.  Backdoor-free.
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_mrc(4'd11, 4'd10);    ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 0
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_cmp_imm(4'd11, 8'd0); ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 1
    ann_hw_sched_prog[ann_hw_sched_prog_len] = 32'd0;                     ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 2 — patched below
    ...
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_b_idx(4'h1, 16, 14);          ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 16
    ...
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_b_idx(4'h1, 29, 27);          ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 29
    ...
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_b_idx(4'h1, 49, 47);          ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 49
    ...
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_b_idx(4'h1, 54, 52);          ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 54
    ...
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_b_idx(4'h1, 58, 34);          ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 58
    ...
    ann_hw_sched_prog[ann_hw_sched_prog_len] = arm_b_idx(4'h1, 72, 70);          ann_hw_sched_prog_len = ann_hw_sched_prog_len + 1; // 72
    ...
    ann_hw_sched_prog[2] = arm_b_idx(4'h1, 2, ann_hw_sched_prog_len - 2);
...
initial begin
    ...
    // ────────────────────────────────────────────────────────────
    //  HW scheduler (primary test) — ANN full-model hardware-owned flow
    //
    //  Flow:
    ...
    //  No back-door pokes — driven purely through the NetFPGA RX/TX interface.
    //  This is the primary full-model acceptance test.
    // ────────────────────────────────────────────────────────────
    begin
        $display("\n--- HW scheduler: ANN full-model hardware-owned flow ---");
        cycle_cnt = 0;
        ...
        if (!(ann_thread_present(2'd0) && ann_thread_present(2'd1) &&
            ...
            // idempotent while the engine is busy, wait targets are shared
            // registers, and CPU DMEM writes during post-processing are
            // deterministic across threads.  If this test TIMEOUTs, the
            // problem is a specific idempotence gap in the CP10 / dma_engine
            // / sm_core kick handling rather than a fundamental architectural
            // limitation of the rx/tx-only flow.
            ann_compute_reference_model();
