 ...
 *   - Mar. 05, 2026: v1.4 — Removed debug_rf ports (sm_core v1.3+).
 *     GPR reads now use override RF read ports. Added K39–K47 CVT tests
 *     with deep CVT pipeline diagnostics.
 */
...
module sm_core_tb;
    ...
    // v1.4: CVT encoding: {OP_CVT, DT, 2'b00, rD, rA, 16'h0}
    //   dt=1: int16 → bf16 (i2f)
    //   dt=0: bf16 → int16 (f2i)
    ...
    // ================================================================
    // GPR Read via Override RF Port (v1.4 — replaces debug_rf)
    // ================================================================
    // Uses force/release on sm_core's ovr_rf_r0_addr_mux to read any
    // GPR from any SP. Safe after kernel_done (TC/BU idle).
    ...
    // Check GPR via override read port
    task check_gpr;
        input [1:0] sp;
        input [3:0] addr;
        input [15:0] expected;
    ...
    // ================================================================
    // Pipeline Monitor — cycle-by-cycle trace (v1.4: SIMT + CVT)
    // ================================================================
    integer cycle_count;
    reg trace_en;
    reg cvt_trace_en;
    ...
    // --- Enhanced trace (v1.4: SIMT + CVT deep diagnostics) ---
    always @(posedge clk) begin
    ...
    // ================================================================
    // Instruction Disassembler (v1.4: added CVT)
    // ================================================================
    task disasm;
        input integer addr;
        input [31:0] inst;
    ...
    //   0x4170 = 15.0    0x42C8 = 100.0

    // Dump SP0-3 RF contents for registers rStart..rEnd
    task dump_rf_range;
        input [3:0] r_start;
        input [3:0] r_end;
        integer ri, si;
    ...
    initial begin
        $display("============================================");
        $display("  SM Core Testbench v1.4 — Starting");
        $display("============================================");
        ...
        begin
            ...
            imem[0]  = enc_mov_tid(4'd0);
            imem[1]  = enc_movi(4'd1, 16'd2);
            imem[2]  = enc_movi(4'd2, 16'd1);
            imem[3]  = enc_movi(4'd5, 16'd0);
        ...
        begin
            ...
            imem[7]  = enc_movi(4'd5, 16'hBBBB);
            imem[8]  = enc_m(`OP_ST, 4'd5, 4'd0, 16'd200);
            imem[9]  = enc_bra(27'd13);
            imem[10] = enc_movi(4'd5, 16'hAAAA);
            imem[11] = enc_m(`OP_ST, 4'd5, 4'd0, 16'd200);
            imem[12] = INST_NOP;
            imem[13] = INST_RET;
        ...
        begin
            ...
            imem[12] = INST_NOP;
            imem[13] = enc_m(`OP_ST, 4'd5, 4'd0, 16'd300);
            imem[14] = INST_RET;
        ...
        begin
            ...
            imem[1] = enc_movi(4'd2, 16'd0);
            imem[2] = enc_m(`OP_ST, 4'd1, 4'd2, 16'd0);
            imem[3] = INST_RET;
        ...
        //    Exact program from soc_tb Test 24.
        begin
            $display("\n--- K45: CVT self-ref rD==rA (SB stalls) ---");
            reset_dut;
            clear_dmem;
            ...
            imem[6] = enc_m(`OP_ST, 4'd2, 4'd0, 16'd1);
            imem[7] = INST_RET;
        ...
        // ── Summary ──────────────────────────────────────
        $display("\n============================================");
        $display("  SM Core Testbench v1.4 — Summary");
